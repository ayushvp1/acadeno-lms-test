// ==========================================================================
// ACADENO LMS — Enrollments Page (US-HR-03)
// ==========================================================================
  ...
  return (
    <div className="hr-container">
      ...
      <div className="filter-bar">
        ...
        <button className="btn-sm btn-primary-sm" onClick={fetchEnrollments} style={{ alignSelf: 'flex-end' }}>
          Apply Filters
        </button>
      </div>
      ...
      <div className="hr-table-wrap">
        {loading ? (
          ...
          <div className="empty-state">No enrollments match the selected filters.</div>
        ) : (
          <table className="hr-table">
            ...
            <tbody>
              {enrollments.map(e => (
                <tr
                  key={e.enrollment_id}
                  onClick={() => navigate(`/hr/enrollments/${e.student_id || e.enrollment_id}`)}
                >
                  <td><code>{e.registration_number}</code></td>
                  <td>
                    <strong>{e.student_name}</strong>
                    <br />
                    <span style={{ fontSize: 12, color: 'var(--gray-text)' }}>{e.email}</span>
                  </td>
                  <td>{e.course_name}</td>
                  <td>{e.batch_name || '—'}</td>
                  <td>{statusBadge(e.enrollment_status, 'enrollment')}</td>
                  <td>{statusBadge(e.payment_status, 'payment')}</td>
                  <td>
